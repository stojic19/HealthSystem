    ...
    public class MedicineSpecificationController : BasePharmacyController
    {
        ...
        public IActionResult MedicineSpecificationRequest(SpecificationDTO dto)
        {
            ...
            Medicine medicine = UoW.GetRepository<IMedicineReadRepository>()
                ...
                .Include(x=>x.MedicinePotentialDangers)
        ...
        private MedicineDTO makeMedicineDTO(Medicine medicine)
        {
            MedicineDTO dto = new MedicineDTO
            {
                ...
                MedicinePotentialDangers = new List<string>(),
                Substances = new List<string>(),
            };
            foreach (SideEffect sideEffect in medicine.SideEffects)
            {
                dto.SideEffects.Add(sideEffect.Name);
            }

            foreach (Reaction reaction in medicine.Reactions)
            {
                dto.Reactions.Add(reaction.Name);
            }

            foreach (Precaution precaution in medicine.Precautions)
            {
                dto.Precautions.Add(precaution.Name);
            }

            foreach (MedicinePotentialDanger medicineMedicinePotential in medicine.MedicinePotentialDangers)
            {
                dto.MedicinePotentialDangers.Add(medicineMedicinePotential.Name);
            }
