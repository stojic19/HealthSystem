...
export class PatientMedicalRecordComponent implements OnInit {
  ...
  refresh(){
    ...
    this.sub = this._service.getFutureAppointments(this.currentUser.userName).subscribe({
      next: (futureAppointments: IAppointment[]) => {
        this.futureAppointments.data = futureAppointments;
        this.changeDetectorRefs.detectChanges();
      }
    });
    this.sub = this._service.getfinishedAppointments(this.currentUser.userName).subscribe({
      next: (finishedAppointments: IFinishedAppointment[]) => {
        this.finishedAppointments.data = finishedAppointments;
        this.changeDetectorRefs.detectChanges();
      },
    });
    this.sub = this._service.getCanceledAppointments(this.currentUser.userName).subscribe({
